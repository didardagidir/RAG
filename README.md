...
![Agentic RAG demo — the Streamlit app answering a multi-hop question and showing its full reasoning trace](docs/demo.png)

*The demo answers a multi-hop question (Tokarev → his university → its founding year → "1755")
while exposing the agent's reasoning trace: decompose, retrieve, grade, generate — with cited
sources. Run it with `streamlit run app/streamlit_app.py`.*
...
The Streamlit app runs the agent over the HotpotQA corpus and shows its full reasoning
trace — sub-questions, retrieval, grade decisions, and any rewrite loops — alongside the
grounded answer and its sources.

```bash
# place the notebook-produced files in ./data:
...
- [x] **Phase 7 — Demo.** Streamlit app exposing the agent's full reasoning trace (sub-questions, retrieval, grade decisions, rewrite loops) over the HotpotQA corpus.
